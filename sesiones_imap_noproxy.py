...
# Variante SIN proxies residenciales: todo el trafico sale por tu IP real.
MODO_SIN_PROXY = True
...
def _listar_csvs_descargas() -> list[Path]:
    ...
    for f in dest_dir.glob("*.csv"):
        # Ignorar temporales de escritura (.email.csv.part.NNNN)
...
def _elegir_csv_preferido(candidatos: list[Path], client_email: str) -> Path | None:
    ...
    def _clave(p: Path):
        try:
            ...
            mtime = 0.0
        # Preferir stem exacto (misma capitalización) y el más reciente
...
def buscar_contrasena_cuenta(correo_solicitado: str) -> str | None:
    ...
    # 1. Exacto en sesiones_imap_cuentas.txt (primera pasada: igualdad con puntos)
    ...
    # 2. Fallback passwords.txt (clave IMAP / genérica): solo match exacto del correo
...
def _invite_arrancar_prefetch_otp(estado: dict, correo: str, *, es_alta: bool) -> None:
    """No-op: sondear /claim antes del PUT cachea KV vacío y retrasa el OTP 20–60 s."""
    return
    ...
    if _invite_hay_pantalla_codigo(page) or _invite_eval_modo_contrasena(page, "existe"):
        if _invite_eval_modo_contrasena(page, "existe"):
            if estado.get("intentos_modo_pwd", 0) < 5:
                ...
                time.sleep(1.5)
                return "progreso"
        # Sin enlace a contraseña → usar código (worker o IMAP)
        if not encontrar_locator_en_frames(page, pwd_selectors):
            ...
            print(f"    [Invitación] [{correo}] Obteniendo código por {canal}...")
            codigo = None
            if usa_worker:
                codigo = _invite_tomar_otp_worker(estado, correo, es_alta=False, max_wait_s=25.0)
            else:
...
def abrir_enlace_familia_con_autocierre(url: str, correo: str, proxy_pe: dict | None = None) -> None:
    ...
    if not proxy_pe or not proxy_pe.get("server"):
        if MODO_SIN_PROXY:
            print(f"    {Color.CYAN}[Navegador] [{correo}] Modo sin proxy: invitación con IP real.{Color.ENDC}")
            proxy_pe = {}
        else:
    ...
    with sync_playwright() as p:
        ...
        def abrir_contexto(proxy, prof_dir):
            ...
            p_serv = (proxy or {}).get("server", "") if proxy else ""
            if MODO_SIN_PROXY or not p_serv:
        ...
        def _rotar_proxy_y_perfil(razon: str) -> bool:
            """Rota proxy PE o reinicia perfil limpio (modo sin proxy). False si no hay IP."""
            nonlocal current_proxy, profile_dir
            if MODO_SIN_PROXY:
                print(f"    {Color.WARNING}[Invitación] [{correo}] {razon}. Modo sin proxy: reiniciando perfil...{Color.ENDC}")
                _cerrar_contexto()
...
def abrir_enlace_restablecimiento_con_autocierre(url: str, correo: str, proxy_pe: dict | None = None) -> bool:
    ...
    if not proxy_pe or not proxy_pe.get("server"):
        if MODO_SIN_PROXY:
            print(f"    {Color.CYAN}[Navegador] [{correo}] Modo sin proxy: restablecimiento con IP real.{Color.ENDC}")
            proxy_pe = {}
        else:
    ...
    with sync_playwright() as p:
        ...
        def _rotar_proxy_y_perfil(razon: str) -> bool:
            nonlocal current_proxy, profile_dir
            if MODO_SIN_PROXY:
                print(f"    {Color.WARNING}[Reset] [{correo}] {razon}. Modo sin proxy: reiniciando perfil...{Color.ENDC}")
                _cerrar_contexto()
                perfil_quemado = profile_dir
                profile_dir = Path(tempfile.gettempdir()) / f"tidal_reset_link_{email_safe}_{random.randint(1000, 9999)}"
                reparar_perfil_corrupto(profile_dir)
        ...
        if pwd_cuenta:
            try:
                # Mostrar huella (no la clave completa) para detectar mismatches de alias Gmail
                ...
                def _rellenar_pwd_reset(locator, valor: str) -> bool:
                    """Rellena password de forma fiable (fill + type + JS) y verifica valor."""
                    if not locator:
                        return False
                    try:
                    ...
                    leido = ""
                    try:
                        leido = (locator.input_value() or "")
                    except Exception:
                        leido = ""
                    if leido == valor:
                        return True
                    # JS + setter nativo (React a veces ignora fill)
                    try:
                        ok = locator.evaluate(
                            """(el, v) => {
                                el.focus();
                ...
                # Preferir name=newPassword (no confundir con confirm)
                ...
                if pwd_new1:
                    if not _rellenar_pwd_reset(pwd_new1, pwd_cuenta):
                        ...
                        # Confirmación (mismo valor exacto)
                        try:
                            ...
                            # Si hay varios password, el 2º suele ser confirm
                            if not pwd_new2:
                                todos = page.locator('input[type="password"]').all()
                                visibles = []
                                for ip in todos:
                                    try:
                                        if ip.is_visible():
                                ...
                                if len(visibles) >= 2:
                                    pwd_new2 = visibles[1]
                            if pwd_new2:
                                if not _rellenar_pwd_reset(pwd_new2, pwd_cuenta):
                                    print(f"    {Color.WARNING}[Reset] [{correo}] Confirmación "
                                          f"de contraseña no coincidió al escribir; reintento...")
                                    _rellenar_pwd_reset(pwd_new2, pwd_cuenta)
                        except Exception:
                            pass

                        # Verificar que ambos campos (si hay) tienen el valor del .txt
                        try:
                        ...
                        # Confirmar éxito (URL / texto) — sin falsos positivos agresivos
                        for _ in range(24):
                            time.sleep(0.5)
                            try:
                                u = (page.url or "").lower()
                            ...
                            if any(x in u for x in ("/login", "signin", "account.tidal")):
                                if "reset" not in u and "newpassword" not in u and "token" not in u:
                                    ...
                                    try:
                                        ...
                                        pass
                            # Error explícito → no marcar OK
                            try:
                                if page.locator("text=Algo salió mal").or_(
                                    page.locator("text=Something went wrong")
                                ).or_(page.locator("text=incorrecta")).or_(
                                    page.locator("text=do not match")
                                ).count() > 0:
                                    success_detected = False
                                    break
                            except Exception:
...
def listar_invitaciones_familiares_buzon(
    ...
    try:
        ...
        if worker_cubre_alias(gmail_user):
            hits = listar_invites_worker(gmail_user, max_age_minutes)
            if hits:
                print(f"    {Color.GREEN}[WORKER]{Color.ENDC} {len(hits)} invitación(es) para {gmail_user}.")
    ...
    try:
        ...
        status, messages = mail.uid("SEARCH", f'(FROM "tidal" SINCE {_imap_since_str(max_age_minutes or 120)})')
...
def _imap_parse_fetch_messages(data) -> dict[int, "email.message.Message"]:
    ...
    out: dict[int, _email.message.Message] = {}
    if not data:
        return out
    pending_uid = None
    for item in data:
        ...
        for chunk in chunks:
            ...
            if b"\n" in chunk and (b"From:" in chunk or b"Subject:" in chunk or b"To:" in chunk):
                try:
                    msg = _email.message_from_bytes(chunk)
                except Exception:
                    pass
    ...
    try:
        ...
        if not solo_link:
            otp_asunto = _imap_otp_rapido_asunto(
                ...
                max_n=n_leer or 8, silencioso=False, carpeta=carpeta,
            )
            if otp_asunto:
                return otp_asunto
            ...
            for uid_b in msg_ids:
                try:
                    ...
                    if not raw:
                        ...
                        continue
                    rec = _headers_destinatario(email.message_from_bytes(raw))
                    if any(
        ...
        for msg_id in msg_ids:
            ...
            # Si solo viene la forma canónica (sin puntos), aceptar solo si no hay hermanos en el texto.
            to_header = (msg.get("To") or "").lower()
            delivered_to = (msg.get("Delivered-To") or "").lower()
            envelope_to = (msg.get("Envelope-To") or "").lower()
            x_original = (msg.get("X-Original-To") or "").lower()
...
def asegurar_proxies_peru(cantidad_necesaria: int = 5) -> list[dict]:
    ...
    if MODO_SIN_PROXY:
        print(f"  {Color.CYAN}[Sin Proxy] Modo IP real: no se cargan proxies de Peru.{Color.ENDC}")
        return []
...
def validar_proxies_opcion13():
    if MODO_SIN_PROXY:
        print(f"\n{Color.CYAN}[Sin Proxy]{Color.ENDC} Esta variante no usa proxies. Opcion 13 no aplica.")
...
class TidalRegisterManager:
    ...
    def rotar_proxy_contexto(self, tipo="NG"):
        ...
        if MODO_SIN_PROXY:
            print(f"\n  [Sin Proxy] [{self.client_email}] Reiniciando navegador con IP real...")
            try:
                if self.context:
                    self.context.close()
            except Exception:
    ...
    def cambiar_a_proxy_peru(self):
        if MODO_SIN_PROXY:
            print(f"\n  [Sin Proxy] [{self.client_email}] Registro inicial finalizado.")
            print(f"  [Sin Proxy] [{self.client_email}] Continuando pago con IP real...")
            self.current_proxy_type = "PE"
            self.use_proxy = False
            self.proxy_ng_server = None
            self.proxy_pe_server = None
    ...
    def asegurar_navegador_abierto(self):
        ...
        if MODO_SIN_PROXY:
            self.use_proxy = False
            print(f"  [Sin Proxy] [{self.client_email}] Usando IP real.")
        elif self.use_proxy:
    ...
    def run_registration(self, cerrar_navegador_al_final=True, timeout_confirmacion_s: float | None = None) -> bool:
        ...
        try:
            ...
            try:
                if (
                    ...
                    print(f"  [Registro] {Color.GREEN}[{self.client_email}] La cuenta SÍ quedó registrada. "
                          f"Se ignora el error OTP y se da el registro por completado.{Color.ENDC}")
                    registro_exitoso = True
                    try:
                        cookies_proxy = self.context.cookies()
                        self.cookies_tidal = [
            ...
            if cerrar_navegador_al_final:
                ...
                # Mantener Chrome abierto (p. ej. upgrade familiar opción 14); liberar solo NG.
            ...
            try:
                ...
                if self._url_indica_cuenta_activa(curr_url):
                    ...
                    if any(x in texto for x in (
                        ...
                        return True
                    # URL de cuenta estable dos lecturas seguidas: no esperar más al texto del DOM.
                    time.sleep(0.25)
                    try:
                        u_stable = (self.page.url or "").lower()
                    except Exception:
                        u_stable = curr_url
                    if self._url_indica_cuenta_activa(u_stable):
                        hay_login = False
                        try:
                            hay_login = bool(encontrar_locator_en_frames(
                                self.page,
    ...
    def abrir_tunemymusic_inmediato(self) -> bool:
        """Abre TuneMyMusic reutilizando Chrome del registro (mismo hilo Playwright)."""
        try:
            from playwright.sync_api import sync_playwright

            def _contexto_vivo() -> bool:
                try:
            ...
            def _lanzar_contexto_tmm(pw):
                ...
                if self.proxy_pe_server:
                    ...
                    if MODO_SIN_PROXY:
                        print(f"  [Sin Proxy] [{self.client_email}] TuneMyMusic con IP real.")
                        proxy_dict = None
                    else:
            ...
            def _rotar_proxy_pe_tmm(razon: str) -> bool:
                if MODO_SIN_PROXY:
                    print(f"  {Color.WARNING}[TuneMyMusic] [{self.client_email}] {razon}. "
                          f"Reiniciando Chrome (sin proxy)...{Color.ENDC}")
            ...
            # Fast-path: reutilizar el Chrome del registro (evita 5–15s de cierre/relanzado).
            if _contexto_vivo():
                print(f"  [TuneMyMusic] [{self.client_email}] Reutilizando Chrome del registro "
                      f"(sin cerrar/relanzar)...")
                try:
                    inyectar_cookies_tmm(self.context, self.client_email)
                except Exception as e:
                    print(f"  [TuneMyMusic] [WARN] Error al cargar cookies de TuneMyMusic: {e}")
                try:
                    nueva = self.context.new_page()
                    nueva.client_email = self.client_email
                    nueva.manager = self
                    self.page = nueva
                    self.page.bring_to_front()
                except Exception:
            ...
            self._tmm_abierto = True
            return True

        except Exception as e:
            print(f"  {Color.FAIL}[TuneMyMusic] [{self.client_email}] ERROR al abrir TuneMyMusic: {e}{Color.ENDC}")
            self._tmm_abierto = False
            return False

    def completar_transferencia_tmm(
        self,
        event_subir_csv: threading.Event,
        cancelar_event: threading.Event | None = None,
    ) -> None:
        """Espera ENTER (mismo hilo Playwright) y ejecuta subida + monitoreo + cierre."""
        transfer_ok = False
        mantener_abierta = False
        try:
            print(f"  [TuneMyMusic] [{self.client_email}] Esperando que prepares la pantalla de selección de archivo...")
            while not event_subir_csv.is_set():
                if cancelar_event is not None and cancelar_event.is_set():
                    print(f"  [TuneMyMusic] [{self.client_email}] Transferencia cancelada; cerrando...")
                    return
                event_subir_csv.wait(timeout=0.5)
            ...
            if not csv_path:
                ...
                if not file_input:
                    ...
                    # Doble chequeo justo antes de set_input_files (evita carrera entre ventanas)
    ...
    def run_tmm_transfer(
        ...
        """Abre TMM (reutiliza Chrome si está vivo) y, tras ENTER, sube el CSV.

        Debe ejecutarse en el mismo hilo que creó el Playwright del registro.
        """
        if not self.abrir_tunemymusic_inmediato():
            # Si falló la apertura y aún hay Chrome del registro, cerrarlo.
            try:
                self.cerrar_navegador(liberar_ng=True, liberar_pe=True)
            except Exception:
                pass
            try:
                ...
                pass
            return
        self.completar_transferencia_tmm(event_subir_csv, cancelar_event=cancelar_event)
        ...
        try:
            ...
            for _intento_nav in range(1, _max_intentos_nav + 1):
                try:
                    ...
                    # Soft ERR_ABORTED: a veces la pestaña ya llegó a /resetpass
                    if es_error_navegacion_abortada(_nav_err) and not es_error_proxy_o_red(_nav_err):
                        try:
                            url_now = (self.page.url or "").lower() if self.page and not self.page.is_closed() else ""
                        except Exception:
                            url_now = ""
                        if "resetpass" in url_now or ("login.tidal.com" in url_now and "pricing" not in url_now):
                            print(f"  [Reset Pass] [{self.client_email}] ERR_ABORTED pero la pestaña "
                                  f"quedó en reset/login ({url_now[:70]}). Se continúa.")
                            try:
                                aceptar_cookies_con_espera(self.page)
                            except Exception:
                                pass
                        ...
                        if _intento_nav < _max_intentos_nav:
                            print(f"  [Reset Pass] [{self.client_email}] Reintentando sin rotar IP "
                                  f"(ERR_ABORTED suave)...")
                            time.sleep(random.uniform(1.5, 2.5))
                            continue

                    # Modo sin proxy: NUNCA consultar el pool PE (está vacío y abortaba el lote).
                    if MODO_SIN_PROXY:
                        if _intento_nav >= _max_intentos_nav:
                            break
                        print(f"  [Sin Proxy] [{self.client_email}] Reiniciando navegador/perfil "
                              f"tras fallo de navegación (sin pool PE)...")
                        try:
                            self.rotar_proxy_contexto()
                        except Exception as e_rot:
                            print(f"  [Reset Pass] [WARN] Reinicio sin proxy falló: {e_rot}")
                            time.sleep(2.0)
                        continue

                    # Con proxy: rotar solo ante fallo real de túnel/conectividad
                    if es_error_proxy_o_red(_nav_err) or "timeout" in err_txt:
                        p_pe = GLOBAL_PE_PROXY_POOL.rotar_y_marcar_bloqueado(self.proxy_pe_server)
                        nuevo = (p_pe or {}).get("server") if p_pe else None
                        if not nuevo:
                            raise RuntimeError(
            ...
            t_claim = threading.Thread(
                target=_claim_enlace_reset, daemon=True, name=f"reset-claim-{self.client_email}"
            )
            t_claim.start()
...
class TidalFamilyInviter:
    ...
    def abrir_navegador(self):
        ...
        if MODO_SIN_PROXY:
            ...
            print("  [Sin Proxy] [Inviter Titular] Conectando con IP real.")
            from playwright.sync_api import sync_playwright
            if not self.playwright:
                self.playwright = sync_playwright().start()
            try:
                ...
                print(f"  [Inviter Titular] [WARN] Fallo lanzamiento: {e}. Reintentando...")
                reparar_perfil_corrupto(self.perfil_dir)
                time.sleep(2.0)
                self.context = self.playwright.chromium.launch_persistent_context(**launch_kwargs)
            self.context.set_default_navigation_timeout(45000)
    ...
    def rotar_proxy_contexto(self, tipo="PE"):
        ...
        if MODO_SIN_PROXY:
            print("  [Sin Proxy] Reiniciando invitador con IP real...")
            self.proxy_pe_server = None
            self.use_proxy = False
    ...
    def _cambiar_a_modo_codigo_si_hay_password(self, titular) -> bool:
        """Si Tidal muestra contraseña, intenta pasar a 'iniciar sesión sin/con código'.

        Devuelve True si tras el intento ya no hay campo password (o nunca lo hubo).
        """
        ...
        # 1) Helpers usados en invitaciones (más fiables con el texto suelto de Tidal)
        try:
            if _invite_eval_modo_contrasena(self.page, "existe"):
                _invite_clic_modo_contrasena(self.page)
                time.sleep(2.0)
                self.page = pagina_vigente(self.page)
    ...
    def _login_titular_con_password(self, titular, password: str) -> bool:
        """Completa el login del titular con contraseña conocida (sesiones_imap_cuentas.txt)."""
        ...
        if not pwd_input:
            return False
        print(f"  [Inviter] [{titular['correo']}] Iniciando sesión con contraseña guardada...")
        try:
            pwd_input.fill("")
            pwd_input.fill(password)
        except Exception:
            try:
                rellenar_campo_humanizado(pwd_input, password)
            except Exception:
                return False
        time.sleep(0.4)
        try:
        ...
        btn_login = esperar_locator_en_frames(
            ...
            [
                ...
                "button:has-text('Sign in')", "button:has-text('Continuar')",
                "button:has-text('Continue')",
            ],
            timeout_s=6.0
        )
        if btn_login:
            try:
                ...
                try:
                    btn_login.evaluate("el => el.click()")
                except Exception:
                    pass
        time.sleep(2.0)
        manejar_bloqueos_e_intervencion(self.page, f"Invitador Titular post-password ({titular['correo']})")
        self.page = pagina_vigente(self.page)
    ...
    def _login_titular_una_vez(self, titular) -> bool:
        """Login titular con bypass de reputación, antibot y reintento ante fallos de red.

        Preferencia: contraseña de sesiones_imap_cuentas.txt. Fallback: código IMAP.
        En MODO_SIN_PROXY reinicia Chrome/IP real; no exige pool PE.
        """
        max_intentos_nav = 4
        ultimo_error = None
        ...
        for intento_nav in range(1, max_intentos_nav + 1):
            try:
                ...
                if intento_nav < max_intentos_nav:
                    if MODO_SIN_PROXY:
                        print(f"  [Inviter] [{titular['correo']}] Login incompleto "
                              f"(intento {intento_nav}/{max_intentos_nav}). "
                              f"Reiniciando navegador (sin proxy)...")
                    else:
                        print(f"  [Inviter] [{titular['correo']}] Login incompleto "
                ...
                if es_error_proxy_o_red(e) and intento_nav < max_intentos_nav:
                    if MODO_SIN_PROXY:
                        print(f"  [Inviter] [{titular['correo']}] Fallo de red ({e}). "
                              f"Reiniciando con IP real ({intento_nav}/{max_intentos_nav})...")
                    else:
    ...
    def _login_titular_flujo(self, titular) -> bool:
        ...
        try:
            ...
            if es_error_proxy_o_red(e_pr):
                raise
            print(f"  [Inviter] [{titular['correo']}] [WARN] Pricing: {e_pr}")
        ...
        try:
            ...
            # Soft abort: a veces la pestaña ya quedó en login
            try:
                u = (self.page.url or "").lower()
            except Exception:
                u = ""
            if not (url_es_login_o_cuenta(u) or "account.tidal.com" in u):
                raise
            print(f"  [Inviter] [{titular['correo']}] goto login abortó pero la pestaña "
                  f"quedó en {u[:70]}. Se continúa.")
        manejar_bloqueos_e_intervencion(self.page, f"Invitador Titular ({titular['correo']})")
        self.page = pagina_vigente(self.page)
        aceptar_cookies_con_espera(self.page)
        ...
        # Preferir contraseña si la tenemos: el modo código a menudo no dispara correo
        # cuando Tidal ya ofreció el campo password (fallo típico de la opción 9).
        pwd_cuenta = buscar_contrasena_cuenta(titular["correo"])
        hay_pwd_ui = bool(encontrar_locator_en_frames(
            self.page, ['input[type="password"]', 'input[name="password"]']
        ))
        if hay_pwd_ui and pwd_cuenta:
            if self._login_titular_con_password(titular, pwd_cuenta):
                return True
            print(f"  [Inviter] [{titular['correo']}] Login por contraseña falló. "
                  f"Probando inicio por código IMAP...")
            if not self._cambiar_a_modo_codigo_si_hay_password(titular):
                print(f"  [Inviter] [{titular['correo']}] No se pudo cambiar a modo código "
                      f"tras fallar la contraseña.")
                return False
        elif hay_pwd_ui and not pwd_cuenta:
        ...
        for intento in range(1, 15):
            ...
            code_or_link = obtener_codigo_via_imap(
                ...
                required_keywords=[
                    "sign-in code", "signin code", "login code", "código de acceso",
                    "codigo de acceso", "verification code", "código de verificación",
                    "codigo de verificacion", "one-time", "código", "codigo", "code",
                    "verific", "acceso", "entrar", "inici", "login", "link",
                ],
                query_exclude=[
                    "invited to a tidal family", "plan tidal family", "tidal family",
                    "plan familiar", "eliminación de tu cuenta", "eliminacion de tu cuenta",
                    "delete your account", "account deletion", "reset your password",
                    "restablecer", "resetpass",
                ],
            ...
            if code_or_link:
                break
            # A mitad: reintentar forzar modo código / reenviar
            if intento in (4, 8):
                try:
                    self._cambiar_a_modo_codigo_si_hay_password(titular)
                    btn_resend = esperar_locator_en_frames(
        ...
        if not code_or_link:
            ...
            if pwd_cuenta and encontrar_locator_en_frames(
                ...
                print(f"  [Inviter] [{titular['correo']}] Sin código IMAP; reintentando con contraseña...")
                return self._login_titular_con_password(titular, pwd_cuenta)
    ...
    def asegurar_login_titular(self, titular) -> bool:
        ...
        if pwd:
            print(f"  [Inviter] Contraseña del titular encontrada en sesiones_imap_cuentas.txt.")
        else:
            print(f"  [Inviter] {Color.WARNING}Sin contraseña en sesiones_imap_cuentas.txt para "
...
def restablecer_contrasenas_tidal(correos=None):
    ...
    if MODO_SIN_PROXY:
        print(f"\n{Color.CYAN}[Sin Proxy] Opcion 9 con IP real.{Color.ENDC}")
        valid_pe_list = []
    else:
...
class TidalAutoLoginManager:
    ...
    def rotar_proxy_contexto(self, tipo="PE"):
        ...
        if MODO_SIN_PROXY:
            print(f"  [Sin Proxy] [{self.client_email}] Reiniciando con IP real...")
            self.proxy_pe_server = None
            self.use_proxy = False
            self.asegurar_navegador_abierto()
            return
    ...
    def handle_download(self, download):
        try:
            ...
            # No marcar completed: sin CSV válido en disco no cuenta como exportación OK
            self.download_completed = False
    ...
    def verificar_destino_del_codigo(self, target_email_clean: str) -> bool:
        ...
        La UI de eliminación a menudo muestra primero el 'Nombre de acceso' (login), no el
        'Correo electrónico' de Información general. No basta con candidatos[0].
        """
        ...
        try:
            # NO lower() el body entero: puede alterar rarezas; normalizamos al comparar
            ...
            for c in candidatos:
                if correos_iguales_exacto(c, login):
                    ...
                    print(f"  [Eliminación] {Color.GREEN}[OK] Se continúa: el código se leerá por IMAP "
                          f"en '{target}' (ya comprobado en passwords.txt).{Color.ENDC}")
                    return True
        ...
        print(f"  [Eliminación] {Color.FAIL}[ALERTA] Correos visibles en la pantalla: "
              f"{', '.join(candidatos)}.{Color.ENDC}")
        print(f"  [Eliminación] {Color.FAIL}Ninguno coincide con el destino IMAP esperado "
              f"'{target or login}'.{Color.ENDC}")
        return False
    ...
    def asegurar_navegador_abierto(self):
        ...
        if MODO_SIN_PROXY:
            self.use_proxy = False
            print(f"  [Sin Proxy] [{self.client_email}] Conectando con IP real.")
        elif self.proxy_pe_server:
            p_serv = self.proxy_pe_server
    ...
    def run_auto_login(self, modo: str = "tmm") -> bool:
        ...
        try:
            ...
            if MODO_SIN_PROXY:
                print(f"  [Navegador] [{self.client_email}] Abriendo Chrome con IP real (sin proxy)...")
            else:
            ...
            for intento_nav in range(1, _max_intentos_nav + 1):
                try:
                    ...
                    print(f"  [Login] {Color.WARNING}[WARN] [{self.client_email}] Intento "
                          f"{intento_nav}/{_max_intentos_nav} de navegación falló ({e_nav})...{Color.ENDC}")

                    # ERR_ABORTED / redirect: reintentar con el MISMO proxy/IP. Solo rotar ante
                    ...
                    if es_error_navegacion_abortada(e_nav) and not es_error_proxy_o_red(e_nav):
                        ...
                        if url_es_login_o_cuenta(actual):
                            print(f"  [Login] [{self.client_email}] Navegación abortada pero la pestaña "
                                  f"quedó en login/cuenta ({actual[:70]}). Se continúa sin rotar IP.")
                            try:
                                manejar_bloqueos_e_intervencion(self.page, "Login Tidal")
                                self.esperar_redireccion_login_o_sesion(12.0)
                                nav_exitoso = True
                    ...
                    if not es_error_proxy_o_red(e_nav) and intento_nav < _max_intentos_nav:
                        print(f"  [Login] [{self.client_email}] Fallo no atribuible a red/proxy. "
                              f"Reintentando sin rotar IP...")
                        time.sleep(random.uniform(1.5, 2.5))
                        continue
                    ...
                    if rotaciones_proxy >= _max_rotaciones_proxy:
                        raise RuntimeError(
                            f"Agotados los {_max_rotaciones_proxy} reinicios de red/navegador para "
                            f"{self.client_email} tras fallos de conectividad."
                        )

                    rotaciones_proxy += 1

                    # Modo sin proxy: NUNCA consultar el pool PE (está vacío y abortaba la opción 15).
                    # ERR_NETWORK_CHANGED (p. ej. cambio de VPN) → reiniciar Chrome/perfil con IP real.
                    if MODO_SIN_PROXY:
                        print(f"  [Sin Proxy] [{self.client_email}] Reinicio "
                              f"{rotaciones_proxy}/{_max_rotaciones_proxy} tras fallo de red "
                              f"(sin pool PE)...")
                        try:
                            self.rotar_proxy_contexto()
                        except Exception as e_rot:
                            print(f"  [Login] [WARN] Reinicio sin proxy falló: {e_rot}")
                            # Fallback mínimo: cerrar y reabrir
                            try:
                                if self.context:
                                    self.context.close()
                            except Exception:
            ...
            print(f"  [Login] [{self.client_email}] Se omite la edición de correo del perfil; "
                  f"se continúa a TuneMyMusic.")

            # Gate final: no abrir TMM si la pestaña de Tidal ya no tiene sesión.
            if not self.es_sesion_activa() and not self.confirmar_sesion_en_perfil(15.0):
            ...
            # Abrir TuneMyMusic en una pestaña aparte con tolerancia a fallos de red/proxy
            ...
            # Cuentas vacías (sin_playlists): TuneMyMusic no ofrece CSV; eso ya es éxito válido.
            ...
            # Opción 10: sin eliminación de cuenta — éxito = CSV exportado o cuenta vacía confirmada.
            if self.download_completed or self.sin_playlists:
                if self.sin_playlists:
                    print(f"\n  [TuneMyMusic] [{self.client_email}] Cuenta vacía: proceso finalizado sin eliminar el perfil.")
                else:
                    print(f"\n  [TuneMyMusic] [{self.client_email}] CSV exportado: proceso finalizado sin eliminar el perfil.")
                self.cerrar_recursos()
                return True
            return self.finalizar_sin_exito("No se descargó el CSV ni se confirmó que la cuenta esté vacía.")
...
def eliminar_cuentas_tidal_automatico_opcion15(correos):
    """Opción 15: login → verificar 'Correo electrónico' del perfil en passwords.txt → eliminar."""
    print(f"\n{Color.BLUE}{Color.BOLD}" + "="*60 + f"{Color.ENDC}")
    print(f"{Color.BLUE}{Color.BOLD}   ELIMINAR CUENTA TIDAL AUTOMÁTICO{Color.ENDC}")
    print(f"{Color.BLUE}{Color.BOLD}" + "="*60 + f"{Color.ENDC}")
    print(f"{Color.CYAN}Flujo: iniciar sesión → leer 'Correo electrónico' del perfil "
          f"(sin cambiarlo) → comprobar IMAP en passwords.txt → eliminar → cerrar Chrome.{Color.ENDC}")
    ...
    num_cuentas = len(correos_lista)
    if MODO_SIN_PROXY:
        print(f"\n{Color.CYAN}[Sin Proxy] Opción 15 con IP real.{Color.ENDC}")
        valid_pe_list = []
    else:
        print(f"\n{Color.CYAN}[Proxies PE] Habilitando proxies de Perú obligatorios (Opción 15)...{Color.ENDC}")
        valid_pe_list = asegurar_proxies_peru(cantidad_necesaria=num_cuentas)
        if not valid_pe_list:
    ...
    print(f"\n{Color.BLUE}{Color.BOLD}" + "="*60 + f"{Color.ENDC}")
    print(f"   RESUMEN — ELIMINAR CUENTA TIDAL AUTOMÁTICO")
    print(f"{Color.BLUE}{Color.BOLD}" + "="*60 + f"{Color.ENDC}")
    print(f" Cuentas procesadas: {total_cuentas}")
    print(f" Inicios de sesión correctos: {total_login}")
...
def registrar_cuentas_tidal(correos):
    ...
    if MODO_SIN_PROXY:
        print(f"\n{Color.CYAN}[Sin Proxy] Opcion 8 con IP real.{Color.ENDC}")
        use_proxy = False
        proxies_pe = []
    else:
        ...
        for line in lines:
            ...
            if en_miembros:
                if "@" in line_clean and "," not in line_clean:
                    ...
                    # nuevo titular sin palabra TITULAR intermedia
                    if actual:
                        titulares.append(actual)
                    actual = _parse_linea_titular(line_clean)
                    en_miembros = False
                continue
            # Línea de metadatos del titular
...
def verificar_csvs_descargas_opcion16(correos: list[str]):
    """Opción 16: comprueba que exista un CSV en descargas/ para cada correo activo del menú."""
    print(f"\n{Color.BLUE}{Color.BOLD}" + "="*60 + f"{Color.ENDC}")
    print(f"{Color.BLUE}{Color.BOLD}   OPCIÓN 16: VERIFICAR CSV EN DESCARGAS/{Color.ENDC}")
    print(f"{Color.BLUE}{Color.BOLD}" + "="*60 + f"{Color.ENDC}")

    if not correos:
        print(f"\n{Color.WARNING}[Info]{Color.ENDC} No hay correos activos. Cárgalos al inicio o con la opción 6.")
        return

    DESCARGAS_DIR.mkdir(parents=True, exist_ok=True)
    print(f"\n  Carpeta: {DESCARGAS_DIR}")
    print(f"  Correos activos a comprobar: {len(correos)}\n")
    ...
    for correo in correos:
        ...
        if not correo or "@" not in correo:
            continue
        # Sin reservas: es solo una lectura de comprobación
        ruta = resolver_csv_cuenta(correo, permitir_alias=True, respetar_reservas=False)
        if ruta and csv_parece_valido(ruta):
            encontrados.append((correo, ruta))
            try:
                nbytes = ruta.stat().st_size
            except Exception:
                nbytes = 0
            marca = "" if ruta.stem.lower() == correo.strip().lower() else " [alias]"
            print(f"  {Color.GREEN}✓{Color.ENDC} {correo} → {ruta.name} ({nbytes} bytes){marca}")
        else:
            faltantes.append(correo)
            print(f"  {Color.FAIL}✗{Color.ENDC} {correo} → falta descargas/{correo}.csv")

    print()
    if faltantes:
        print(f"{Color.FAIL}{Color.BOLD}>>> FALTAN CSV PARA {len(faltantes)}/{len(correos)} CORREO(S): <<<{Color.ENDC}")
        for c in faltantes:
            print(f"  {Color.FAIL}✖ {c}{Color.ENDC}")
        print(f"{Color.WARNING}Coloca los archivos como 'descargas/correo@dominio.csv' "
              f"y vuelve a comprobar.{Color.ENDC}\n")
    else:
        print(f"{Color.GREEN}{Color.BOLD}>>> TODO CORRECTO: Hay CSV válido en 'descargas/' "
              f"para los {len(encontrados)} correo(s) activo(s). <<<{Color.ENDC}\n")
...
def crear_cuentas_familiares_automatico_opcion14():
    ...
    if MODO_SIN_PROXY:
        print(f"\n{Color.CYAN}[Sin Proxy] Opcion 14 con IP real.{Color.ENDC}")
        valid_ng_list = []
        valid_pe_list = []
        batch_size = min(10, max(1, num_cuentas))
    else:
...
def menu_principal():
    ...
    print(f"{Color.CYAN}{Color.BOLD}   [MODO SIN PROXY — IP REAL]{Color.ENDC}")
    print(f"{Color.BLUE}{Color.BOLD}" + "="*60 + f"{Color.ENDC}")
    ...
    while True:
        ...
        print(" 13. Validar y verificar lista de proxies (Nigeria / Perú) [N/A sin proxy]")
        ...
        if opcion in ("1", "2", "3"):
                ...
                if opcion == "1":
                    ...
                    for intento in range(1, 9):
                        ...
                        if codigo:
                            break
                        if intento < 8:
                            print(f"    {Color.WARNING}[WORKER] Esperando código (intento {intento}/8)...{Color.ENDC}")
                            time.sleep(2.5)
                    if codigo:
                        print(f"{Color.GREEN}{Color.BOLD}>>> CÓDIGO ENCONTRADO: {codigo} <<<{Color.ENDC}")
                    else:
            ...
            if enlaces_reset:
                if MODO_SIN_PROXY:
                    print(f"\n{Color.CYAN}[Sin Proxy] Abriendo restablecimientos con IP real "
                          f"(auto-relleno de contraseña + cierre al éxito)...{Color.ENDC}")
                    proxies_pe = []
                else:
                    print(f"\n{Color.CYAN}[Proxies PE] Habilitando proxies de Perú obligatorios...{Color.ENDC}")
                    proxies_pe = asegurar_proxies_peru(cantidad_necesaria=len(enlaces_reset))
                    if not proxies_pe:
                        print(f"\n{Color.FAIL}[Error]{Color.ENDC} No hay proxies de Perú válidos.")
                        proxies_pe = None
                if MODO_SIN_PROXY or proxies_pe is not None:
            ...
            if enlaces_map:
                ...
                if MODO_SIN_PROXY:
                    print(f"\n{Color.CYAN}[Sin Proxy] Abriendo invitaciones con IP real "
                          f"(auto-login + aceptar + cerrar al éxito)...{Color.ENDC}")
                    proxies_pe = []
                else:
                    print(f"\n{Color.CYAN}[Proxies PE] Habilitando proxies de Perú obligatorios...{Color.ENDC}")
                ...
                with ThreadPoolExecutor(max_workers=min(20, len(enlaces_map))) as executor:
                    ...
                    futures = [executor.submit(procesar_invitacion_hilo, idx + 1, item)
                               for idx, item in enumerate(items)]
                    for future in as_completed(futures):
                        try:
                            future.result()
