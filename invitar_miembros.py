...
8) opcional: invita al plan Familiar al miembro indicado en el archivo de invitar miembros
   (p. ej. invitar_miembros.txt o «Invitar miembros.txt» en Windows), misma línea que emails/LINKS;
   pulsa «Invitar a un familiar», rellena el correo y envía la invitación.
...
DEFAULT_INVITAR_MIEMBROS_FILE = SCRIPT_DIR / "invitar_miembros.txt"


def _candidatos_archivos_invitar_miembros() -> tuple[Path, ...]:
    """
    ...
    d = SCRIPT_DIR
    return (
        d / "invitar_miembros.txt",
        d / "Invitar miembros.txt",
        d / "invitar miembros.txt",
        d / "Invitar_miembros.txt",
    )


def _resolver_archivo_invitar_miembros(ruta_explicita: Path | None) -> Path | None:
    ...
    for p in _candidatos_archivos_invitar_miembros():
        if p.is_file():
            return p
    return None
...
def verificar_y_reintentar_fase_anterior(
    ...
    *,
    ...
    elif fase_interrumpida in ("familia", "invitar_miembro"):
...
def _identificar_fase_interrumpida(momento: str) -> str:
    ...
    if "correo" in momento_lower or "fase 2" in momento_lower:
        ...
        return "familia"
    elif "invitar" in momento_lower or "fase 8" in momento_lower:
        return "invitar_miembro"
    else:
        return "desconocida"
...
def manejar_captcha_tid_si_aplica(
    ...
    *,
    ...
    while True:
        ...
        # Redirigir ventanas afectadas a tidal.com/pricing
        print("\n  [Bypass] Redirigiendo ventanas afectadas a https://tidal.com/pricing...")
            ...
            for t in trabajos:
                if t["n"] == n_af:
                    try:
                        ...
                        print(f"    Error al redirigir ventana [{n_af}] a pricing: {e_nav}")
...
def cargar_sesiones(
    ...
    emails_path: Path,
    passwords_path: Path,
    invitar_miembros_path: Path | None = None,
    *,
    cargar_invitar_miembros: bool = True,
) -> list[dict]:
    ...
    if cargar_invitar_miembros:
        ruta_inv = _resolver_archivo_invitar_miembros(invitar_miembros_path)
        if invitar_miembros_path is not None and ruta_inv is None:
            print(
                f"Aviso: el archivo de --invitar-miembros no existe o no es un fichero: "
                f"{invitar_miembros_path}"
            )
            lineas_invitar = []
        elif ruta_inv is not None:
            lineas_invitar = leer_lineas_si_existe(ruta_inv)
            if lineas_invitar:
                print(
                    f"Fase 8: {len(lineas_invitar)} línea(s) con correos a invitar "
                    f"desde «{ruta_inv.name}»."
                )
        else:
            lineas_invitar = []
    else:
    ...
    for i in range(n):
        ...
        invitar_miembro = lineas_invitar[i].strip() if i < len(lineas_invitar) else ""
        if not email:
            print(
        ...
        sesiones.append(
            {
                ...
                "email": email,
                "password": password,
                "invitar_miembro": invitar_miembro,
            }
        )
    return sesiones
...
def _invitar_miembro_uia(wnd, email_miembro: str) -> bool:
    ...
    try:
        ...
        for d in wnd.descendants(control_type="Edit"):
            name = (d.element_info.name or "").lower()
            if "correo" in name or "email" in name or "address" in name:
                edit_correo = d
                break

        if not edit_correo:
            # Intentar buscar un botón para abrir la invitación si está colapsada
            btn_abrir = None
            for d in wnd.descendants(control_type="Button"):
                name = (d.element_info.name or "").lower()
                if "invitar a un familiar" in name or "invite a family" in name or "añadir" in name or "add" in name:
                    btn_abrir = d
                    break
            if btn_abrir:
                print(f"      Haciendo clic en el botón '{btn_abrir.element_info.name}' para abrir la sección de invitación...")
                ...
                for d in wnd.descendants(control_type="Edit"):
                    ...
                    if "correo" in name or "email" in name or "address" in name:
                        ...
                        break
                if not edit_correo:
                    edits = wnd.descendants(control_type="Edit")
                    if edits:
                        edit_correo = edits[0]
        ...
        edit_correo.type_keys("^a{BACKSPACE}", with_spaces=True)
        time.sleep(0.2)
        # Escribir el email
        edit_correo.type_keys(email_miembro, with_spaces=True)
        time.sleep(1.5)

        # 2. Buscar el botón "Invitar" o "Invite"
        btn_invitar = None
        for d in wnd.descendants(control_type="Button"):
            name = (d.element_info.name or "").lower()
            if name.strip() in ("invitar", "invite"):
                btn_invitar = d
                break
        ...
        if not btn_invitar:
            for d in wnd.descendants(control_type="Button"):
                name = (d.element_info.name or "").lower()
                if "invitar" in name or "invite" in name:
                    btn_invitar = d
                    break
        ...
        try:
            enabled = btn_invitar.is_enabled()
        except Exception:
            enabled = True
        ...
        try:
            ...
            if val.strip() != "" and val.strip() == email_miembro:
                print("      ⚠️ El correo sigue en el campo de texto. Verificando si hay errores...")
                success = False
        except Exception:
            pass
...
def invitar_miembro_plan_familiar_tid(
    ...
    *,
    ...
    if not input_loc:
        print("    [Invitar] Formulario de invitación no visible. Buscando botón para abrirlo...")
        for frame in _frames_visibles(page):
            for rx in nombres_boton_abrir:
                try:
                    btn = frame.get_by_role("button", name=rx).first
                    if btn.count() > 0 and btn.is_visible(timeout=500):
                        print(f"    [Invitar] Pulsando botón '{rx.pattern}' para abrir formulario.")
                        hacer_clic_humanizado(page, btn)
                        time.sleep(pausa_s + 0.3)
                        break
                    lnk = frame.get_by_role("link", name=rx).first
                    if lnk.count() > 0 and lnk.is_visible(timeout=500):
                        print(f"    [Invitar] Pulsando enlace '{rx.pattern}' para abrir formulario.")
            ...
            # Re-verificar si apareció el input
            for sel in selectores_input:
                try:
                    loc = frame.locator(sel).first
    ...
    # 5) Verificar si se envió o si hay error
    alertas_exito = (
        re.compile(r"invitaci[oó]n\s+enviada", re.I),
        re.compile(r"invitation\s+sent", re.I),
        re.compile(r"enviada\s+con\s+[eé]xito", re.I),
        re.compile(r"exitosamente", re.I),
    )
    alertas_error = (
        re.compile(r"ya\s+(est[aá]|pertenece)", re.I),
        re.compile(r"already\s+(in|member)", re.I),
    ...
    while time.monotonic() < deadline:
        ...
        for frame in _frames_visibles(page):
            ...
            for rx in alertas_error:
                try:
                    loc = frame.get_by_text(rx)
                    if loc.count() > 0 and loc.first.is_visible(timeout=200):
                        print(f"    [Invitar] ⚠️ Alerta de error detectada: '{loc.first.inner_text()}'")
                        return False
                except Exception:
                    continue
        time.sleep(0.3)
...
def ejecutar_playwright(
    ...
    *,
    ...
    omitir_fase_invitar_miembros: bool = False,
    ...
    with sync_playwright() as p:
        ...
        try:
                ...
                trabajos.append(
                    {
                        ...
                        "perfil": perfil,
                        "n": i + 1,
                        "invitar_miembro": (s.get("invitar_miembro") or "").strip(),
                        "url": url,
            ...
            if not omitir_fase_invitar_miembros and any(
                (t.get("invitar_miembro") or "").strip() for t in trabajos
            ):
                print(
                    "\n=== Fase 8 — invitar miembro al plan Familiar ==="
                )
                for j, t in enumerate(trabajos):
                    perfil, page, n = t["perfil"], t["page"], t["n"]
                    imiem = (t.get("invitar_miembro") or "").strip()
                    if not imiem:
                        print(f"  [{n}/{total}] {perfil}: (sin correo a invitar en esta línea del archivo)")
                        continue
                    print(f"  [{n}/{total}] {perfil} — invitar miembro: {imiem}", flush=True)
                    try:
                        if page.is_closed():
                        ...
                        if invitar_miembro_plan_familiar_con_reintentos(
                            ...
                            print(
                                "    OK: Invitación enviada correctamente; "
                                "verifica en TIDAL que el miembro figure como invitado."
                            )
                        else:
                            print(
                                "    No se pudo completar la invitación (¿ya invitado, límite alcanzado?)."
                            )
                    except Exception as e:
                        print(f"    Error: {e}")
                ...
                comprobar_captcha_post_fase(
                    trabajos,
                    "después de fase 8 — invitar miembro",
                    pausa_s=0.3,
                    captcha_cfg=cfg_captcha,
                )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        dest="invitar_miembros",
        help=(
            "Un correo de miembro a invitar por línea, mismo orden que LINKS/emails. "
            "Si omites esta opción, se busca en la carpeta del script, en este orden: "
            "invitar_miembros.txt, «Invitar miembros.txt», invitar miembros.txt, Invitar_miembros.txt."
        ),
    )
    parser.add_argument(
        "--omitir-fase-invitar-miembros",
        action="store_true",
        dest="omitir_fase_invitar_miembros",
        help="No ejecutar la fase 8 (invitar miembro al plan Familiar en TIDAL).",
    )
    parser.add_argument(
        "--usar-incognito",
        action="store_true",
    ...
    parser.add_argument(
        "--invitar-miembro-reintentos",
        type=int,
        default=2,
        dest="invitar_miembro_reintentos",
        help="Fase 8: reintentos del flujo de invitación (por defecto 2).",
    )
    parser.add_argument(
        "--invitar-miembro-pausa",
        type=float,
        default=0.45,
        dest="invitar_miembro_pausa",
        help="Fase 8: segundos entre pasos de la UI (por defecto 0.45).",
    )
    parser.add_argument(
        "--delay-invitar-miembro",
        type=float,
        default=0.25,
        dest="delay_invitar_miembro",
    ...
    sesiones = cargar_sesiones(
        ...
        args.emails,
        args.passwords,
        invitar_miembros_path=args.invitar_miembros,
        cargar_invitar_miembros=not args.omitir_fase_invitar_miembros,
    )
    if not sesiones:
        sys.exit(1)
    ...
    if args.solo_subprocess:
        ...
        for i, s in enumerate(sesiones):
            ...
            password = s["password"]
            url = s["url"]
            invitar_miembro = s.get("invitar_miembro", "").strip()
            ...
            # 8. Invitar miembro si aplica
            if invitar_miembro and not args.omitir_fase_invitar_miembros:
                print(f"  [Fase 8] Intentando invitar miembro al plan familiar: {invitar_miembro}")
                if _invitar_miembro_uia(wnd, invitar_miembro):
    ...
    ejecutar_playwright(
        ...
        omitir_fase_invitar_miembros=args.omitir_fase_invitar_miembros,
        invitar_miembro_reintentos=args.invitar_miembro_reintentos,
        invitar_miembro_pausa_s=args.invitar_miembro_pausa,
        delay_entre_invitar_miembro=args.delay_invitar_miembro,
